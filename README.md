...
[`start-codex.sh`](start-codex.sh) and [`start-gemini.sh`](start-gemini.sh) contain the author's own absolute paths such as `/Users/user/...` and a fixed Bun path. Treat them as personal examples, not portable startup scripts.

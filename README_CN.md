...
| 多 Agent 群聊协作 | &mdash; | &mdash; | 共享上下文 |
...
把 `@claude-bot` 和 `@codex-bot` 拉进同一个 Telegram 群。让 Claude review 代码——Codex 通过共享上下文读到 Claude 的回复，给出自己的意见。
...
两种让 AI agent 互相对话的方式——协议不同，场景不同：

| 层 | 协议 | 方式 | 场景 |
|---|------|------|------|
| **终端** | MCP | 内置 `codex mcp-server` + `claude mcp serve`，零代码 | CC ↔ Codex 在终端互调 |
| **TG 群聊** | 共享上下文 | 本项目的共享上下文存储 | 多 bot 同群协作 |

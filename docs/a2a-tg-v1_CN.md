...
- **版本：** 1（本文）
- **线上版本标签：** `a2a/v1`
  - *历史说明：* 当前实现在信封里打的是 `protocol_version: "a2a/v1"`。未来 v1.1 会迁移到 `a2a-tg/v1`，跟官方 A2A payload 视觉上区分更清晰。在此之前，**只有在下面定义的 `/a2a/message` 传输路径上**出现的 `a2a/v1` 才算 A2A-TG，不是官方 A2A。
...
| 字段                   | 类型    | 必填 | 说明 |
|------------------------|---------|------|------|
| `protocol_version`     | string  | 是   | `"a2a/v1"`（见 §1） |
...
| 版本  | 日期     | 说明 |
|-------|----------|------|
| v1    | 2026-04  | 初版。对应 telegram-ai-bridge 3.1.0 的实现。 |
| v1.1  | 计划中   | 把线上 `protocol_version` 从 `a2a/v1` bump 到 `a2a-tg/v1`，身份更清晰。无语义变化。 |

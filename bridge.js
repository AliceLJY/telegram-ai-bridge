...
async function processPrompt(ctx, prompt) {
  ...
  try {
    ...
    // 注入 bridge 行为指令
    const bridgeHint = "[系统提示: 你通过 Telegram Bridge 与用户对话。当用户要求截图、生成文件、或发送已有文件时，只需在回复中写出文件的完整绝对路径（如 /Users/user/Desktop/file.png），bridge 会自动检测路径并将文件发送给用户。绝对不要自己调用 curl/Telegram Bot API 来发送文件。]\n\n";
